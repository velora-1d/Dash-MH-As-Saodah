<x-app-layout>
    <div class="space-y-6">
        ...
        <div style="background: #fff; border-radius: 1rem; border: 1px solid #e2e8f0; overflow: hidden;">
            ...
            <form action="{{ route('hr.staff.update', $staff->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div style="padding: 2rem;">
                    <div class="fi-grid fi-grid-2">
                        <x-form-group label="Nomor Induk / NIK" name="nip">
                            <input type="text" name="nip" value="{{ old('nip', $staff->nip) }}" placeholder="Contoh: 123456789"
                                class="fi-input @error('nip') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Nama Lengkap (Beserta Gelar)" name="name" :required="true">
                            <input type="text" name="name" value="{{ old('name', $staff->name) }}" placeholder="Contoh: Budi Santoso, S.Kom." required
                                class="fi-input @error('name') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Posisi / Jabatan" name="position" :required="true">
                            <input type="text" name="position" value="{{ old('position', $staff->position) }}" placeholder="Contoh: Staf Tata Usaha" required
                ...
                <div style="padding: 1.25rem 2rem; border-top: 1px solid #f1f5f9; display: flex; justify-content: flex-end; gap: 0.75rem; background: #fafbfc;">
                    ...
                    <button type="submit" style="display: inline-flex; align-items: center; padding: 0.625rem 1.5rem; font-size: 0.8125rem; font-weight: 700; color: #fff; background: linear-gradient(135deg, #0ea5e9, #3b82f6); border: none; border-radius: 0.625rem; cursor: pointer; transition: all 0.15s;" onmouseover="this.style.transform='translateY(-1px)'" onmouseout="this.style.transform=''">
