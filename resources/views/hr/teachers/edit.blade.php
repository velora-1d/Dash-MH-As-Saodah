<x-app-layout>
    <div class="space-y-6">
        ...
        <div style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 50%, #a78bfa 100%); border-radius: 1rem; overflow: hidden; position: relative;">
            ...
            <div style="padding: 2rem; position: relative; z-index: 10;">
                ...
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <div>
                        <h2 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.25rem; color: #fff; margin: 0;">Ubah Data Guru</h2>
                        <p style="font-size: 0.8125rem; color: rgba(255,255,255,0.7); margin-top: 0.125rem;">Perbarui informasi detail profil guru di sistem kepegawaian.</p>
                    </div>
                </div>
        ...
        <div style="background: #fff; border-radius: 1rem; border: 1px solid #e2e8f0; overflow: hidden; width: 100%;">
            ...
            <form action="{{ route('hr.teachers.update', $teacher->id) }}" method="POST" style="padding: 1.5rem;">
                @csrf
                @method('PUT')
                <div style="display: grid; grid-template-columns: 1fr; gap: 1.25rem;">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $teacher->name) }}" placeholder="Contoh: Ahmad Dahlan, S.Pd." required style="width: 100%; padding: 0.625rem 1rem; font-size: 0.8125rem; border: 1px solid #e2e8f0; border-radius: 0.5rem; outline: none; transition: border-color 0.15s ease;" onfocus="this.style.borderColor='#6366f1'" onblur="this.style.borderColor='#e2e8f0'">
                        @error('name')
                            <p style="margin: 0.375rem 0 0; font-size: 0.75rem; color: #ef4444;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="position" value="{{ old('position', $teacher->position) }}" placeholder="Contoh: Wali Kelas 1A" required style="width: 100%; padding: 0.625rem 1rem; font-size: 0.8125rem; border: 1px solid #e2e8f0; border-radius: 0.5rem; outline: none; transition: border-color 0.15s ease;" onfocus="this.style.borderColor='#6366f1'" onblur="this.style.borderColor='#e2e8f0'">
                        @error('position')
                            <p style="margin: 0.375rem 0 0; font-size: 0.75rem; color: #ef4444;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" required style="width: 100%; padding: 0.625rem 1rem; font-size: 0.8125rem; border: 1px solid #e2e8f0; border-radius: 0.5rem; outline: none; background: #fff; cursor: pointer;">
                            ...
                            <option value="nonaktif" {{ old('status', $teacher->status) == 'nonaktif' ? 'selected' : '' }}>Non-Aktif / Cuti</option>
                        </select>
                        @error('status')
                            <p style="margin: 0.375rem 0 0; font-size: 0.75rem; color: #ef4444;">{{ $message }}</p>
                        @enderror
