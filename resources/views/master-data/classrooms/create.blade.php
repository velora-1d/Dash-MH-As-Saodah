                ...
                <div style="padding: 2rem;">
                    <div class="fi-grid fi-grid-2">
                        ...
                        <x-form-group label="Nama Kelas" name="name" :required="true">
                            <input type="text" name="name" id="name" required placeholder="Misal: Kelas 1A" value="{{ old('name') }}"
                                class="fi-input @error('name') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Wali Kelas" name="wali_kelas" hint="Opsional">
                            <input type="text" name="wali_kelas" id="wali_kelas" placeholder="Nama guru wali kelas" value="{{ old('wali_kelas') }}"
                                class="fi-input @error('wali_kelas') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Tarif Infaq/SPP" name="infaq_nominal" :required="true" hint="Nominal bulanan per siswa">
                            <x-money-input name="infaq_nominal" :value="old('infaq_nominal', 0)" placeholder="50.000" />
                        </x-form-group>
                    </div>
                </div>
