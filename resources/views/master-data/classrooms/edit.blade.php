            ...
            <form action="{{ route('classrooms.update', $classroom->id) }}" method="POST">
                ...
                <div style="padding: 2rem;">
                    <div class="fi-grid fi-grid-2">
                        <x-form-group label="Tingkatan Kelas" name="level" :required="true">
                            <select id="level" name="level" required class="fi-input fi-select @error('level') fi-error @enderror">
                                <option value="" disabled>-- Pilih Tingkat --</option>
                                @for($i = 1; $i <= 6; $i++)<option value="{{ $i }}" {{ old('level', $classroom->level) == $i ? 'selected' : '' }}>Kelas {{ $i }}</option>@endfor
                            </select>
                        </x-form-group>

                        <x-form-group label="Nama Kelas" name="name" :required="true">
                            <input type="text" name="name" id="name" required placeholder="Misal: Kelas 1A" value="{{ old('name', $classroom->name) }}"
                                class="fi-input @error('name') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Wali Kelas" name="wali_kelas" hint="Opsional">
                            <input type="text" name="wali_kelas" id="wali_kelas" placeholder="Nama guru wali kelas" value="{{ old('wali_kelas', $classroom->wali_kelas) }}"
                                class="fi-input @error('wali_kelas') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Tarif Infaq/SPP" name="infaq_nominal" :required="true">
                            <x-money-input name="infaq_nominal" :value="old('infaq_nominal', $classroom->infaq_nominal)" placeholder="0" :required="true" />
                        </x-form-group>
                    </div>
                </div>
                ...
                <div style="padding: 1.25rem 2rem; border-top: 1px solid #f1f5f9; display: flex; align-items: center; justify-content: flex-end; gap: 0.75rem; background: #fafbfc;">
                    <a href="{{ route('classrooms.index') }}" style="display: inline-flex; align-items: center; padding: 0.625rem 1.25rem; font-size: 0.8125rem; font-weight: 600; color: #64748b; border: 1.5px solid #e2e8f0; border-radius: 0.625rem; text-decoration: none; transition: all 0.15s;" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='transparent'">Batal</a>
