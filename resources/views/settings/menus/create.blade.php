    ...
    <div class="space-y-6">
                ...
                <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 1rem;">
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                        ...
                        <div style="width: 44px; height: 44px; background: rgba(255,255,255,0.2); backdrop-filter: blur(10px); border-radius: 0.75rem; display: flex; align-items: center; justify-content: center; border: 1.5px solid rgba(255,255,255,0.3);">
                            <svg style="width: 22px; height: 22px; color: #fff;" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                        </div>
                        <div>
                            <h2 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.25rem; color: #fff; margin: 0;">Tambah Menu Baru</h2>
                            <p style="font-size: 0.8125rem; color: rgba(255,255,255,0.7); margin-top: 0.125rem;">Tambahkan item navigasi baru ke dalam sidebar.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div style="background: #fff; border-radius: 1rem; border: 1px solid #e2e8f0; overflow: hidden;">
            <div style="padding: 1.25rem 1.5rem; border-bottom: 1px solid #f1f5f9; display: flex; align-items: center; gap: 0.5rem;">
                ...
                <h4 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 0.875rem; color: #1e293b; margin: 0;">Form Data Menu</h4>
            </div>
                ...
                <div style="padding: 2rem;">
                    <div class="fi-grid fi-grid-2">
                        <x-form-group label="Nama Menu" name="name" :required="true">
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Dashboard Utama"
                                class="fi-input @error('name') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Route Name" name="route_name" :required="true">
                            <input type="text" name="route_name" value="{{ old('route_name') }}" required placeholder="Contoh: dashboard / reports.index"
                                class="fi-input @error('route_name') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Group / Kategori (Header)" name="group_name">
                            <input type="text" name="group_name" value="{{ old('group_name') }}" placeholder="Contoh: Basis Data Utama (Kosongkan jika tidak ada)"
                        ...
                        <x-form-group label="Kode Icon SVG (hanya elemen path)" name="icon_svg" :required="true" class="fi-grid-full">
                            <textarea name="icon_svg" rows="3" required placeholder="Contoh: <path stroke-linecap='round' ... />"
                                class="fi-input fi-textarea @error('icon_svg') fi-error @enderror" style="font-family: monospace;">{{ old('icon_svg') }}</textarea>
                        </x-form-group>
