            ...
            <div style="padding: 2rem; position: relative; z-index: 10;">
                ...
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <div>
                        ...
                        <p style="font-size: 0.8125rem; color: rgba(255,255,255,0.7); margin-top: 0.125rem;">Perbarui transaksi wakaf: {{ $transaction->transaction_number }}</p>
                    </div>
                </div>
                ...
                <div style="padding: 2rem;">
                    <div class="fi-grid fi-grid-2">
                        <x-form-group label="Nama Donatur" name="donor_name" :required="true">
                            <input type="text" name="donor_name" id="donor_name" required list="donor-list" placeholder="Ketik atau pilih donatur"
                                value="{{ old('donor_name', $transaction->donor->name ?? '') }}" autocomplete="off"
                                class="fi-input @error('donor_name') fi-error @enderror">
                        ...
                        <x-form-group label="No. HP Donatur" name="donor_phone" hint="Opsional">
                            <input type="text" name="donor_phone" id="donor_phone" placeholder="08xx"
                                value="{{ old('donor_phone', $transaction->donor->phone ?? '') }}"
                                class="fi-input @error('donor_phone') fi-error @enderror">
                        </x-form-group>

                        <x-form-group label="Nominal Wakaf (Rp)" name="amount" :required="true">
                            <x-money-input name="amount" :value="old('amount', $transaction->amount)" placeholder="1000000" :required="true" />
                        </x-form-group>

                        <x-form-group label="Tujuan Wakaf" name="wakaf_purpose" :required="true">
                            <input list="purpose_names" name="wakaf_purpose" id="wakaf_purpose" required
                                value="{{ old('wakaf_purpose', $transaction->purpose->name ?? '') }}" placeholder="Pilih atau ketik tujuan wakaf..."
                        ...
                        <x-form-group label="Tanggal" name="date" :required="true">
                            <input type="date" name="date" id="date" required value="{{ old('date', $transaction->date) }}"
                        ...
                        <x-form-group label="Keterangan" name="description" hint="Opsional">
                            <input type="text" name="description" id="description" placeholder="Catatan tambahan"
                                ...
                                class="fi-input @error('description') fi-error @enderror">
                        </x-form-group>
                    </div>
                </div>
