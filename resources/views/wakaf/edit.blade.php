<x-app-layout>
    <div class="space-y-6">
        <div style="background: linear-gradient(135deg, #059669 0%, #10b981 50%, #34d399 100%); border-radius: 1rem; overflow: hidden; position: relative;">
            ...
            <div style="padding: 2rem; position: relative; z-index: 10;">
                ...
                <div style="display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <div>
                        <h2 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.25rem; color: #fff; margin: 0;">Edit Data Wakaf</h2>
                        <p style="font-size: 0.8125rem; color: rgba(255,255,255,0.7); margin-top: 0.125rem;">Perbarui data transaksi wakaf untuk ID: {{ $transaction->transaction_number }}</p>
                    </div>
                </div>
        ...
        <div style="background: #fff; border-radius: 1rem; border: 1px solid #e2e8f0; overflow: hidden;">
            <form action="{{ route('wakaf.update', $transaction->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div style="padding: 2rem; display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem;">
                    ...
                    <div>
                        ...
                        <input type="text" name="donor_name" id="donor_name" required list="donor-list" placeholder="Ketik atau pilih donatur" value="{{ old('donor_name', $transaction->donor->name ?? '') }}" style="width: 100%; box-sizing: border-box;" autocomplete="off">
                    ...
                    <div>
                        ...
                        <input type="text" name="donor_phone" id="donor_phone" placeholder="08xx" value="{{ old('donor_phone', $transaction->donor->phone ?? '') }}" style="width: 100%; box-sizing: border-box;">
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" required min="1000" step="1000" placeholder="1000000" value="{{ old('amount', $transaction->amount) }}" style="width: 100%; box-sizing: border-box;">
                    ...
                    <div>
                        ...
                        <div style="position: relative;">
                            <input list="purpose_names" name="wakaf_purpose" id="wakaf_purpose" required value="{{ old('wakaf_purpose', $transaction->purpose->name ?? '') }}" placeholder="Pilih atau ketik tujuan wakaf baru..." style="width: 100%; box-sizing: border-box; padding-left: 2.25rem;">
                    ...
                    <div>
                        ...
                        <select name="cash_account_id" id="cash_account_id" required style="width: 100%; box-sizing: border-box;">
                            ...
                            @foreach ($cashAccounts as $ca)<option value="{{ $ca->id }}" {{ old('cash_account_id', $transaction->cash_account_id) == $ca->id ? 'selected' : '' }}>{{ $ca->name }}</option>@endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="category_id" id="category_id" required style="width: 100%; box-sizing: border-box;">
                            ...
                            @foreach ($categories as $c)<option value="{{ $c->id }}" {{ old('category_id', $transaction->category_id) == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>@endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="date" required value="{{ old('date', $transaction->date) }}" style="width: 100%; box-sizing: border-box;">
                    ...
                    <div>
                        ...
                        <input type="text" name="description" id="description" placeholder="Catatan tambahan" value="{{ old('description', $transaction->description) }}" style="width: 100%; box-sizing: border-box;">
                    </div>
                </div>
